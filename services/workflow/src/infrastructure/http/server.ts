...
import { registerWorkflowRoutes } from '../../interface/routes/workflow.routes';
import { registerHealthRoutes } from '../../interface/routes/health.routes';

// =============================================================================
// HTTP server assembly for the Workflow service.
// =============================================================================
...
export async function buildApp(opts: BuildAppOptions): Promise<FastifyInstance> {
  ...
  const service = new WorkflowService(prisma, opts.events ?? new NoopEventPublisher());
  ...
  await app.register(fastifySwagger, {
    openapi: {
      info: {
        ...
        description: 'Business process automation for the Enterprise AI CRM.',
        version: '1.0.0',
      },
      servers: [{ url: '/api/v1' }],
      components: {
  ...
  await app.register(
    async (api) => registerWorkflowRoutes(api, service, authenticate),
    { prefix: '/api/v1' },
  );
