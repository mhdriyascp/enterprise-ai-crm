...
async function start(): Promise<void> {
  ...
  const shutdown = async (signal: string): Promise<void> => {
    logger.info({ signal }, 'Shutting down workflow service');
    await app.close();
  ...
  try {
    ...
    logger.info({ port: config.PORT, host: config.HOST }, 'Workflow service started');
  } catch (error) {
    logger.error({ error }, 'Failed to start workflow service');
    process.exit(1);
  }
}
