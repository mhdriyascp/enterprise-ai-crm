...
async function start(): Promise<void> {
  ...
  const shutdown = async (signal: string): Promise<void> => {
    logger.info({ signal }, 'Shutting down lead service');
    await app.close();
  ...
  try {
    ...
    logger.info({ port: config.PORT, host: config.HOST }, 'Lead service started');
  } catch (error) {
    logger.error({ error }, 'Failed to start lead service');
    process.exit(1);
  }
}
